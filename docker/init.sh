...
poetry run jsng "ident=j.core.identity.new(\"default\", \"$dummy_tname\", \"$dummy_email\", network=\"$explorer_network\", admins=$ADMINS); ident.register(); ident.save()"

# Create the default threebot
poetry run jsng 'j.servers.threebot.new("default"); j.servers.threebot.default.save()'
poetry run jsng "j.servers.threebot.default.packages.add(path='/owncloud_deployer/jumpscale/packages/owncloud')"
